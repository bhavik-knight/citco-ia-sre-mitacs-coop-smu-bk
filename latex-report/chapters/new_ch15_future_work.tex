...
\begin{tabularx}{\textwidth}{@{\hspace{\tabcolsep}}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{0.22\textwidth}@{\hspace{\tabcolsep}}}
\caption{Auto-Healing Agent --- Initial Remediation Catalogue} \label{tab:autohealing_catalogue} \\
\hline
...
\rowcolor{tablerowB} MSK consumer lag $>$ 10{,}000 messages & Scale ECS consumer service (+1 task) & SNS + JIRA ticket (auto-created via JIRA API) \\
...
\rowcolor{tablerowA} ECS CPU $>$ 80\% sustained 5 min & Trigger ECS service auto-scaling policy (\texttt{RegisterScalableTarget}) & SNS if already at max task count \\
\rowcolor{tablerowB} RPA bot missing heartbeat (future) & Call UiPath Orchestrator API to restart robot job & SNS + support ticket \\
\hline
\end{tabularx}
