...
The work I delivered in this co-op placement establishes a monitoring foundation --- not merely an endpoint. I have already defined several planned extensions in the JIRA
backlog, scoped for implementation during the Mitacs BSI renewal
(September~1 -- December~31, 2026).


\section{Distributed Tracing with X-Ray}
\label{sec:future_xray}
...
\section{Auto-Healing Agent}
\label{sec:future_autohealing}
...
appropriate recovery action from a configurable remediation catalog, and
...
\begin{enumerate}
    ...
    \item The alarm fires an SNS message to the \texttt{ia-sre-auto-healing-topic}
    ...
    \item The Healer Lambda looks up the alarm name in a
          \textbf{remediation catalog} --- a DynamoDB table mapping alarm
...
\subsection{Remediation Catalog (Initial Scope)}
...
\begin{table}[htbp]
\centering
\caption{Auto-Healing Agent --- Initial Remediation Catalog}
\label{tab:autohealing_catalog}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.22\textwidth} >{\raggedright\arraybackslash}p{0.45\textwidth} >{\raggedright\arraybackslash}p{0.22\textwidth}}
\hline
...
\hline
\rowcolor{tablerowA} ECS task desired $\neq$ running & \texttt{ecs:UpdateService} (force new deployment) & SNS alert to SRE \\
\rowcolor{tablerowB} MSK consumer lag $>$ 10{,}000 messages & Scale ECS consumer service (+1 task) & SNS + JIRA ticket \\
\rowcolor{tablerowA} Lambda error rate $>$ 1\% over 5 min & Trigger Step Functions diagnostic workflow; capture last 50 error log lines to S3 & SNS alert with S3 log link \\
\rowcolor{tablerowB} SQS queue age $>$ 1 hour & Invoke queue-processor Lambda with \texttt{force=true} parameter & SNS alert \\
\rowcolor{tablerowA} ECS CPU $>$ 80\% sustained 5 min & Trigger ECS service auto-scaling policy & SNS if at max tasks \\
\rowcolor{tablerowB} RPA bot missing heartbeat & Call UiPath Orchestrator API to restart robot job & SNS + support ticket \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Idempotency.} Every recovery action must be safe to invoke
          multiple times. The DynamoDB remediation catalog tracks the last
    ...
    \item \textbf{Bounded autonomy.} The Healer acts on a whitelist of
          known-safe recovery actions. Any alarm with no matching catalog
...
\subsection{Relation to the PoC}
...
\section{RPA Bot CloudWatch Integration}
\label{sec:future_rpa_cw}
...
\section{Error Budget Burn-Rate Alerting}
\label{sec:future_error_budget}
...
\label{sec:future_tag_compliance}
