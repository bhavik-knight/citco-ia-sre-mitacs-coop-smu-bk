...
code environments housing 119 Lambda functions, 22 ECS clusters, 83 EC2
...
(IISS-507) that exposes the resource fleet programmatically, and the SRE
...
\begin{itemize}
    ...
    \item \textbf{No cross-environment aggregation}: A single
          ...
          across the five budget code environments (\texttt{rpacfs1},
...
dimension---across the five budget code environments. While technically
...
\subsection{IA Monitoring Platform Sub-Stories (IISS-482 to IISS-486)}
...
the five budget code environments. This scan revealed a critical tagging
...
as a separate time series, covering all 119~functions across all budget
...
\begin{enumerate}
    ...
    \item For each template (Lambda alarms, ECS alarms, SQS alarms, MSK
          ...
          all current resources in the specified budget code environment.
...
new budget code or resource type requires implementing one class and
...
\midrule
\texttt{GET /api/v1/ecs} & All ECS clusters across all budget codes \\
\midrule
...
service categories, the generated YAML grew to \textbf{14,720~lines}
...
\begin{itemize}
    \item No size limit --- templates can grow arbitrarily as additional
          service categories are onboarded.
...
policy requiring each budget code environment to have a unique
...
\begin{itemize}
    \item \textbf{Fleet-scale coverage}: The provisioner ensures every
          Lambda function in every budget code environment has alarms
...
budget code environments (\texttt{rpacfs1}, \texttt{ia-bpo}, \texttt{automationhub},
\texttt{meridian}, \texttt{citcoworks}) and the Lambda, ECS, and EC2 workloads they
...
\begin{itemize}
    \item \textbf{Ho Hoi Leung (Horace) --- RPA Support.}
          ...
          how Blue Prism (BPM) and UiPath Cloud Orchestrator are monitored, and
...
\textbf{51 Lambda functions} then visible across the two budget codes to their
...
and \texttt{meridian} budget codes. The extension work surfaced two structural
...
\begin{itemize}
    ...
    \item \textbf{Top-K and metric filter constraint.} A proposed central
          ...
          five budget codes. This was blocked by an IAM permission boundary:
...
By the end of Phase~1, the full Lambda inventory stood at \textbf{119 functions}
across all five budget codes, documented in Confluence. The \texttt{SEARCH()}
...
I implemented Lambda discoverers for all five budget codes, confirming the
119-function inventory documented in Phase~1. The \texttt{rpacfs1} discoverer
...
I implemented ECS discoverers for all 22 clusters across five budget codes.
...
in the shared-budget-code environments, a governance decision documented in
...
keeping the endpoint within the 10-second NFR target across the full 119-function
fleet.
...
\begin{itemize}
    ...
    \item \textbf{Property-based tests (Hypothesis)} --- tag-parsing logic and
          ...
          combinations.
    \item \textbf{Edge case tests} --- malformed tags, empty budget code environments,
...
Blue Prism (BPM) and UiPath bots, responding to service desk tickets,
...
\begin{itemize}
    ...
    \item \textbf{BPM Orchestrator health checks} --- robot session status,
