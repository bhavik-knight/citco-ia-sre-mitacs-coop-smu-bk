...
The week of Apr~13 was designated as a formal knowledge-transfer week.
...
Apr~17, the Meridian platform suffered a significant processing delay: a surge
...
approximately two hours on Apr~7. I provisioned the dashboard via
...
what had been understood informally since the Meridian Incident: the IA team
...
\begin{enumerate}
    ...
    \item \textbf{\texttt{ia-sre-msk-alarms}} --- MSK consumer lag alarm on
          ...
          Meridian Incident invisible until business users flagged it.
\end{enumerate}
...
Percentile statistics (p50, p99) are unavailable in this mode.
...
directly implementing the alarm that would have detected the Meridian Incident.
...
states into a single OK/ALARM indicator per budget code
environment, displayed on Row~12 of the dashboard.
...
3.5-hour business-team lag of Apr~17 to a projected sub-30-second alarm fire
time.
...
Rows~7--8 added EC2 monitoring using the AppName tag as the primary
discovery dimension --- the fallback pattern I established in IISS-483 to handle
the \texttt{rpacfs1} tag mismatch (where BudgetCode carried the value
"CFS/RPA" rather than rpacfs1). The EC2 row covered
...
because OpenSearch was the bottleneck in the Meridian Incident. I wired a JVM heap
...
The SQS monitoring feature (IISS-482) I implemented on Jun~12 was a
...
The dashboard reached its production-ready state on Jun~18 with the deployment
...
Work on IISS-507 began on Jun~11 --- one week before IISS-487 was formally
...
boto3 for all AWS resource discovery, Pydantic for response model
validation, and serverless-wsgi for Lambda deployment.
...
used the AppName fallback pattern: a primary filter on
...
"Shared Cloud Artifacts" for the BudgetCode dimension, a
...
discriminate between the two environments. I formally designated the AppName tag
...
the repository README and enforced by a tag-compliance CloudWatch alarm.
...
discovered resource carried five fields: name, type,
\texttt{budget\_code}, tags (as a free-form dictionary), and
arn for direct AWS Console linkage.
...
Figure~\ref{fig:concurrency_model} illustrates how the event loop, thread pool,
and boto3 clients interact to achieve parallel discovery across all 13 AWS service types.
...
\begin{lstlisting}[language={}, basicstyle=\ttfamily\scriptsize, frame=single,
  rulecolor=\color{smubrand}, backgroundcolor=\color{white}, numbers=none]
+----------------------------------------------------------+
|  Lambda Handler / Flask Request (synchronous)             |
|                                                          |
|  asyncio.run(_get_resource_counts(...))                  |
|       |                                                  |
|       v                                                  |
|  +-----------------------------------------------+      |
|  |  Event Loop                                   |      |
|  |                                               |      |
|  |  asyncio.gather(                              |      |
|  |    run_in_executor(pool, discover, "lambda"), |      |
|  |    run_in_executor(pool, discover, "sqs"),    |      |
|  |    run_in_executor(pool, discover, "ec2"),    |      |
|  |    ... (13 concurrent tasks)                  |      |
|  |  )                                            |      |
|  +-----------------------------------------------+      |
|       |                                                  |
...
|  |  [T1:SQS] [T2:EC2] [T3:ECS] [T4:RDS] [...]   |      |
|  |      |        |        |        |              |      |
|  |    boto3   boto3    boto3    boto3              |      |
|  +-----------------------------------------------+      |
|  Result: all 13 services discovered in parallel;         |
|  total latency = slowest single service (~<10s)          |
+----------------------------------------------------------+
\end{lstlisting}
\caption{Inventory API --- Concurrent Discovery Engine (asyncio + ThreadPoolExecutor)}
...
By Jul~14, my test suite reached \textbf{562 automated tests} with
...
I dedicated the final weeks of Aug to runbook updates, Confluence documentation
for the dashboard operating model, and structured knowledge transfer sessions with
Horace and Kri. The Mitacs BSI renewal for the Sep~1 -- Dec~31, 2026
...
At approximately 9~AM on Jun~8, the CAIS Deadline process failed with a
...
process had failed on Jun~3 and succeeded on Jun~4 (when the container had
been restarted) and failed again on Jun~8 (when a new container picked up
...
On Jul~1, INNOCAP and SAMLMOS fund tasks were found stuck in the queue with
...
\rowcolors{2}{tablerowA}{tablerowB}
\begin{tabularx}{\textwidth}{lp{1.5cm}Xp{4.5cm}p{1.0cm}}
\hline
\rowcolor{smubrand}\textcolor{white}{\textbf{JIRA}} & \textcolor{white}{\textbf{Date}} & \textcolor{white}{\textbf{Description}} & \textcolor{white}{\textbf{Resolution}} & \textcolor{white}{\textbf{Hours}} \\
\hline
...
% ────────────────────────────────────────────────────────────
\section{Development Repositories}
\label{sec:dev_repositories}
% ────────────────────────────────────────────────────────────
...
\hline
IISS-435  & RPACFS1 CloudWatch pipeline health dashboard \\
IISS-455  & Lambda monitoring CloudWatch dashboard (51→119 functions) \\
IISS-461  & Lambda inventory Confluence page \\
IISS-469  & Improved centralized Lambda dashboard; Top-K filter analysis \\
IISS-482  & IA Infrastructure Monitoring Platform (parent) \\
IISS-483  & EC2 monitoring dashboard; tag mismatch discovery \\
IISS-484  & ECS monitoring dashboard; Container Insights cost analysis \\
IISS-485  & EKS monitoring dashboard; AUTOMATIONHUB scope \\
IISS-486  & Export to S3 (scheduled Lambda, Excel export) \\
IISS-487  & Amazon Managed Grafana SRE Dashboard (primary deliverable, 140h) \\
IISS-488  & Auto-Healing Pipeline (SNS → Lambda remediation) \\
IISS-489  & Meridian SRE Observability Roadmap \\
IISS-504  & Extend inventory API for AUTOMATIONHUB \\
IISS-505  & Extend inventory API for \texttt{meridian} \\
IISS-506  & \texttt{meridian} Monitoring Infrastructure (gap analysis) \\
IISS-507  & IA Infrastructure Inventory API (Flask async REST API) \\
IISS-508  & Backend --- discoverer registry pattern \\
IISS-509  & Frontend --- dashboard integration widgets \\
IISS-510  & Extend inventory API for \texttt{citcoworks} \\
IISS-824  & ML/LLM RPA Status Agent documentation and improvements \\
IISS-837--844 & X-Ray distributed tracing (7 sub-tasks) \\
IISS-845--873 & Log group discovery and observability (10 sub-tasks) \\
IISS-538  & MESO high-memory autoscale hotfix \\
IISS-706  & CAIS Deadline UCM credential validation hotfix \\
IISS-741  & MESO month-end stuck tasks (Critical) \\
IISS-753  & VPL IPV UI selector failure \\
IISS-876  & VPL Pricing GTL incorrect booking \\
\hline
\end{tabularx}
\end{table}
