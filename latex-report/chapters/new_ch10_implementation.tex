...
% ────────────────────────────────────────────────────────────
\section{Work Stream 4: RPA LLM Agent --- Ask Citco RPA (IISS-824)}
...
+-------------------------------v------------------------------+
|  COMPUTE                                                      |
|  ECS Fargate  FastAPI (core-smart-rpa-llm-ecr-image)         |
|  - LDAP auth  - OpenAI-format streaming  - Usage tracking    |
+-------------------------------+------------------------------+
                                |  invoke_agent()
+-------------------------------v------------------------------+
|  BEDROCK AGENT  (Claude)                                      |
|                                                               |
|  Action Groups (Lambda)     Knowledge Base (OpenSearch)       |
|  get-project-code            49 SharePoint libraries          |
|  get-process-status-info     ~5,000 wiki pages                |
|  get-le-assignment           1024-dim FAISS/HNSW vectors      |
|  post-create-sdm-ticket      HYBRID search (vector + BM25)    |
+--------------------------------------------------------------+
                                |
+-------------------------------v------------------------------+
|  DATA SOURCES                                                 |
|  SharePoint IRCOEKB  Databricks Event Service  BPM API       |
|  (wiki pages)        (process/project master)  (client/fund) |
+--------------------------------------------------------------+
\end{lstlisting}
...
As part of IISS-824, I uploaded the NavChecklist process documentation
...
\begin{itemize}
    ...
    \item \textbf{Test suite}: 194 unit tests at 99\% code coverage
          (v1.1.x series)
