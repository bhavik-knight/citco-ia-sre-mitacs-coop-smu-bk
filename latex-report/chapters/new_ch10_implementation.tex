...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth, trim=0 0 0 80, clip]{figures/fig-amg-architecture.png}
\caption[AMG Observability Platform --- Architecture Diagram]{AMG Observability Platform --- Architecture Diagram (from documentation)}
...
I built a centralised Amazon Managed Grafana workspace with 17 dashboard rows and panels,
...
I organised the Grafana AMG platform into a four-layer architecture that separates data collection, visualization, alerting, and infrastructure provisioning concerns.
...
I built the Grafana AMG workspace with \textbf{17~dashboard rows} organised by
monitored service category. Table~\ref{tab:dashboard_rows} summarises the
...
\textbf{Throttles} (capacity limit breaches). I added a colour-coded legend below each panel
...
utilisation (both average and maximum) across all tasks, enabling
...
current function fleet at deploy time and synthesise alarm resources
programmatically:
...
\begin{enumerate}
    ...
    \item It reads parameterised CloudFormation YAML templates I stored in an
    ...
    \item It synthesises CloudFormation \texttt{AWS::CloudWatch::Alarm}
...
to enable reuse across DEV, UAT, and PROD without code changes. I externalised the budget
...
integration I configured enables service map visualisation and trace waterfall
...
registering one entry --- zero changes to routing, serialisation, or
test infrastructure.
...
\rowcolor{tablerowA} \texttt{GET /api/v1/s3} & S3 buckets per budget code \\
...
\texttt{asyncio.gather()}, reducing total API latency by parallelising
...
I documented and analysed the KB ingestion pipeline, which I found runs daily in two stages.
...
\rowcolor{tablerowA} L23 & No alarm acknowledgement or snooze & Critical \\
...
\begin{itemize}
    ...
    \item \textbf{Infrastructure Monitoring Dashboard}:
          ...
          layout, Dynatrace-style honeycomb health visualisations,
...
\begin{itemize}
    \item \textbf{CitcoHoneycomb}: A D3 hexbin-based health grid where
          each hexagon represents one resource instance, coloured by
          real-time health status.

    \item \textbf{CitcoServiceMap}: A Cytoscape.js topology visualisation
          replacing the crash-prone Grafana X-Ray plugin.

    \item \textbf{CitcoTraceWaterfall}: A custom Recharts component for
          X-Ray trace span visualisation.
\end{itemize}
...
(Sep--Dec~2026), allowing the Grafana AMG dashboard I built to stabilise in
...
\caption{Design Decision Summary} \label{tab:design_decisions} \\
\hline
...
\hline
\endfoot
\rowcolor{tablerowA} Template storage & S3 (from Secrets Manager) & +\texttt{s3:GetObject} permission; $\sim$50\,ms latency \\
\rowcolor{tablerowB} Tag disambiguation & AppName mandatory secondary key & Tags governance obligation; missing tag = invisible resource \\
\rowcolor{tablerowA} Metrics data source & CloudWatch native plugin only & No p50/p99 in GROUP BY mode; AVG/MAX as proxies \\
\rowcolor{tablerowB} Alarm provisioning & Dynamic Lambda provisioner & Sequential deploy dependency; Lambda failure halts pipeline \\
\hline
\end{tabularx}
