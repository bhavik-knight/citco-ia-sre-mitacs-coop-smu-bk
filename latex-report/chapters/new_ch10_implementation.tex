...
The three deliverables of my co-op placement form a coherent four-layer
...
Figure~\ref{fig:amg_arch_diagram} provides a rendered view of the same
architecture from the project documentation.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig-amg-architecture.png}
\caption{AMG Observability Platform --- Architecture Diagram (from documentation)}
\label{fig:amg_arch_diagram}
\end{figure}

The \textbf{infrastructure layer} is the monitored subject: six budget
...
from the infrastructure layer. Amazon CloudWatch serves as the sole data source.
This was a deliberate architectural decision (see Section~\ref{sec:decision_cloudwatch_only})
that eliminates the operational overhead of a self-managed Prometheus stack.
CloudWatch Metric Insights SQL provides per-resource aggregation, enabling
multi-service panels without requiring one panel per resource.

The \textbf{visualization and alerting layer} splits between two components.
Amazon Managed Grafana (AMG) renders the multi-row dashboard for human operators.
CloudWatch Alarms with SNS routing fires notifications when metrics breach
their SLO-aligned thresholds.

The \textbf{consumers layer} includes three groups: operators who use the
Grafana dashboard during incident response, the IA Resources Inventory REST
API that exposes the AWS service fleet programmatically, and the SRE team's
email distribution list that receives SNS alarm notifications.

% ────────────────────────────────────────────────────────────
\section{CloudWatch Dashboard Platform}
...
simultaneously). Both are documented in full in
...
\caption{CloudWatch Lambda Dashboard --- Live Screenshot (ia-resources-monitoring)}
...
and \texttt{meridian} budget codes, and introduced CloudWatch Logs Insights 
...
% ────────────────────────────────────────────────────────────
\section{Grafana AMG Observability Platform}
\label{sec:grafana_amg_platform}
% ────────────────────────────────────────────────────────────

This was my primary deliverable over significant duration. 
It delivered a centralized Amazon Managed Grafana workspace 
with various dashboard rows and panels, dynamic CloudWatch alarms,
and a Lambda-backed IaC provisioner --- all built
...
\begin{itemize}
    ...
    \item \textbf{IAM role model}: A \textbf{16-role deployment} (one role
          per AWS account, covering all six IA platform accounts) built on a
          \textbf{three-role IAM chaining architecture} provides the Grafana
          service account with cross-account read access across all IA AWS accounts.
          Each role grants the minimum permissions required: \texttt{cloudwatch:GetMetricData},
          ...
          architecture required because \texttt{CTM-Developer} cannot
...
\caption{IAM Role Architecture --- Three-Role Model for Grafana AMG Deployment}
...
The Grafana AMG workspace hosts \textbf{17~dashboard rows} organized by
...
\hline
1 & Health Overview & Color-coded stat tile per service (Lambda, SQS, MSK, EC2, ECR, EKS, ECS, RDS, DocumentDB, Redis, Neptune, OpenSearch); critical/warning alarm counts \\
...
9 & ECS Services & CPU and memory utilization per cluster and service; Container Insights task-level metrics \\
...
screenshots from the deployed Grafana AMG dashboard, demonstrating the
Health Overview, Lambda monitoring, and ECS monitoring rows.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig-grafana-overview-row.png}
\caption{Grafana AMG Dashboard --- Health Overview Row (Live Screenshot)}
...
\caption{Grafana AMG Dashboard --- Lambda Monitoring Row (Live Screenshot)}
\label{fig:grafana_lambda}
\end{figure}

The Lambda Monitoring Row (Figure~\ref{fig:grafana_lambda}) provides
...
Below the table, four time-series panels track the four golden signals:
...
\textbf{Throttles} (capacity limit breaches). The legend below each panel
enables drill-down to individual functions, with color-coding to identify
anomalous patterns.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig-grafana-ecs-row.png}
\caption{Grafana AMG Dashboard --- ECS Monitoring Row (Live Screenshot)}
\label{fig:grafana_ecs}
\end{figure}

The ECS Monitoring Row (Figure~\ref{fig:grafana_ecs}) surfaces
Container Insights metrics for all ECS services in the selected cluster.
The service table shows cluster assignment, desired count, and running
count --- highlighting any discrepancies that indicate failed deployments
or scaling issues. The four time-series panels display CPU and memory
...
The standard form uses \texttt{SELECT ... FROM ... GROUP BY} to return
metrics for each resource as a separate time series, covering all resources
without requiring one panel per function. The \texttt{WHERE} clause can
filter by tag or dimension to scope results to specific platforms.
...
\begin{itemize}
    ...
    \item \textbf{Composite alarms}: Platform-level health signals that
          ...
          OK/ALARM indicator per budget code environment.
\end{itemize}
...
\caption{Alarm Notification Flow --- CloudWatch to SNS to Email Distribution}
...
\caption{CI/CD Deployment Pipeline --- CodeCommit to AMG Deployment Flow}
...
As part of the observability stack, I integrated AWS X-Ray tracing to
provide distributed trace visibility across the IA platform. The X-Ray
integration enables service map visualization and trace waterfall
analysis --- capabilities that contribute to the 60\% Dynatrace parity
achieved by the CloudWatch + Grafana + X-Ray stack (see
...
\caption{AWS X-Ray Service Map --- Distributed Trace Topology (Live Screenshot)}
...
X-Ray tracing is currently enabled in DEV environment only; UAT and PROD
enablement is pending senior approval due to the additional CloudWatch
costs associated with trace segment storage.

% ────────────────────────────────────────────────────────────
\section{IA Resources Inventory API}
\label{sec:inventory_api}
% ────────────────────────────────────────────────────────────

The inventory API delivered a \textbf{Flask REST API} that I built for programmatic discovery
...
Figure~\ref{fig:inventory_api_arch} shows the high-level architecture of
the Inventory API, illustrating how discoverers query AWS services and
return unified resource models.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\textwidth]{figures/fig-inventory-api-architecture.png}
\caption{IA Resources Inventory API --- Architecture Overview}
\label{fig:inventory_api_arch}
\end{figure}
...
\rowcolors{2}{tablerowA}{tablerowB}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.30\textwidth}>{\raggedright\arraybackslash}p{0.65\textwidth}}
\hline
...
Stage~1 filters by BudgetCode (which is shared between both platforms),
...
\caption{Resource Discovery Flow --- Discoverer Registry and Two-Stage Filter}
...
rotations as part of the IA team's \textbf{Follow-the-Sun model}. I held the
North America slot (11~AM--7~PM~EST) once every four weeks, serving as the
primary contact for RPA incidents during that window. This was not project
work with JIRA tickets --- it was ongoing operational responsibility with
Service Desk tickets, email triage, and ad-hoc troubleshooting.
...
\begin{itemize}
    \item \textbf{Week 8} (May~4--8): First rotation. Familiarization with
          ...
          Shadowed Horace on Corporate Actions ticket triage.

    \item \textbf{Week 12} (Jun~1--5): MESO hotfix week. Encountered
          IISS-538 (ECS autoscaling gap) during routine monitoring ---
          this led to a code fix, not just ticket resolution.

    \item \textbf{Week 20} (Jul~27--31): IISS-876 (VPL Pricing GTL booking
...
\begin{itemize}
    \item \textbf{IISS-706 (CAIS Deadline)}: UCM upload failure with
    ...
    \item \textbf{IISS-538 (MESO ECS)}: Tasks stuck, not autoscaling.
          ...
          to the IISS-487 alarm set.

    \item \textbf{IISS-741 (MESO Critical)}: INNOCAP and SAMLMOS funds
    ...
    \item \textbf{IISS-753 (VPL IPV)}: \AE xeo pricing window
...
\begin{itemize}
    \item The MESO ECS incident (IISS-538) led to adding the
          \texttt{ECS\_CPUSaturation} alarm to the Grafana dashboard.

    \item The CAIS credential failure (IISS-706) reinforced the importance
...
\texttt{core-smart-rpa-llm-ecr-image}). My engagement began as a research
and documentation task but expanded to production bug-fixing after log
...
developers to be on-call for routine questions.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\textwidth]{figures/fig-rpa-llm-architecture.png}
\caption{Ask Citco RPA --- RAG Architecture (IISS-824)}
...
The KB ingestion pipeline runs daily in two stages:

\begin{enumerate}
    \item \textbf{12:00 UTC --- \texttt{IRCOEKBExtractLambda}}: Syncs
          49 SharePoint IRCOEKB wiki libraries ($\sim$5,000 pages) to S3
          as plain-text files with SharePoint metadata preserved.
    \item \textbf{14:00 UTC --- \texttt{RPALLMKBIngestionFunction}}: Triggers
...
Static documents (PDFs, process guides) are uploaded separately via an
idempotent upload script (\texttt{scripts/upload\_kb\_content.py}) that
compares content hashes before writing to S3.

\subsection{Production Reliability Issues Identified and Fixed}

My engagement began with CloudWatch log analysis. Analysis of
\textbf{93 days of production logs} (May~5 -- Aug~5, 2026; 148 Lambda
...
\subsection{Knowledge Base Enrichment: NavChecklist Documents}
...
\begin{itemize}
    ...
    \item \textbf{Kill switch}: SSM Parameter Store kill switch pattern
          (\texttt{/{Env}/rpa-llm/ircoekb/process-run-status}: RUN/STOP)
          implemented across IRCOEKB, Event Store, and Mapping Lambdas
    \item \textbf{X-Ray tracing}: AWS Lambda Powertools Tracer
          instrumented on all functions (enabled in DEV; pending approval
          for UAT/PROD)
\end{itemize}

Versions v1.1.1 through v1.1.32 were released during Aug~3--20, 2026.

\subsection{Proposed: Hybrid Graph-RAG Architecture}

During the codebase analysis I identified a structural limitation of
...
as a graph layer alongside the existing OpenSearch vector store. The
graph would model the entity relationship network (client--fund--process--platform),
enabling the Bedrock agent to traverse relationships for multi-hop queries
...
entities. The dual-path retrieval would improve answer accuracy for
complex operational queries while maintaining the existing RAG pipeline
for straightforward procedural lookups. This proposal will be submitted
as a formal IISS follow-on ticket.
...
Work Stream~6 is in active development, extending the existing
\texttt{ia-config-portal} application with two new dashboard pages.
During this co-op placement period, I completed an \textbf{architectural
implementation plan} (IISS-919-IMPLEMENTATION-PLAN.md) and began
initial implementation. Development is continuing during the co-op placement
extension period (Sep--Dec~2026).

\subsection{Planning Document Deliverable}

The primary deliverable for this work stream is a comprehensive
865-line implementation plan that documents:

\begin{itemize}
    \item \textbf{Gap analysis}: 45 documented Grafana AMG limitations
          with severity ratings, identifying which gaps justify a
          custom UI solution.

    \item \textbf{Technology stack decisions}: Evaluation of charting
    ...
    \item \textbf{Backend API design}: Four new Lambda handlers
    ...
    \item \textbf{Task breakdown}: 12 implementation tasks covering
...
The Grafana AMG dashboard built in Work Stream~2 achieved approximately
60\% Dynatrace feature parity, but 45 architectural limitations were
identified. The most critical gaps driving the custom UI proposal:

\begin{table}[!htbp]
\centering
\caption{Critical Grafana AMG Limitations Identified}
\label{tab:grafana_limitations}
\rowcolors{2}{tablerowA}{tablerowB}
\begin{tabularx}{\textwidth}{lXl}
\hline
\rowcolor{smubrand}\textcolor{white}{\textbf{ID}} & \textcolor{white}{\textbf{Limitation}} & \textcolor{white}{\textbf{Severity}} \\
\hline
L1 & CloudWatch 10 TPS hard throttle --- panels error on every load & Critical \\
L9 & X-Ray service map crashes on load --- requires Edit mode workaround & Critical \\
L18 & Table filter does not cross-filter other panels & Critical \\
L21 & No topology / honeycomb / Dynatrace-style health view & Critical \\
L23 & No alarm acknowledgement or snooze & Critical \\
L31 & Cannot embed AMG in other portals (iframe blocked) & Medium \\
\hline
\end{tabularx}
\end{table}
...
The planning document proposes two new dashboard pages within the
existing \texttt{ia-config-portal} (built by Soundarya Venkataraman
and Sridhar Vadla):

\begin{itemize}
    \item \textbf{Infrastructure Inventory Dashboard (IISS-509)}:
          Frontend for the Inventory API built in Work Stream~3,
          displaying resources across all platforms with filtering
          and Excel export capabilities.

    \item \textbf{Infrastructure Monitoring Dashboard (IISS-919)}:
          Config-driven monitoring interface with drag-and-drop panel
          layout, Dynatrace-style honeycomb health visualizations,
          and X-Ray service map integration.
\end{itemize}

Key planned components include:

\begin{itemize}
    \item \textbf{CitcoHoneycomb}: D3 hexbin-based health grid where
          each hexagon represents one resource instance, colored by
          real-time health status.

    \item \textbf{CitcoServiceMap}: Cytoscape.js topology visualization
          replacing the crash-prone Grafana X-Ray plugin.

    \item \textbf{CitcoTraceWaterfall}: Custom Recharts component for
...
\begin{table}[!htbp]
\centering
\caption{Work Stream 6 --- Status Summary}
\label{tab:ws6_status}
\rowcolors{2}{tablerowA}{tablerowB}
\begin{tabularx}{\textwidth}{lXl}
\hline
\rowcolor{smubrand}\textcolor{white}{\textbf{Item}} & \textcolor{white}{\textbf{Description}} & \textcolor{white}{\textbf{Status}} \\
\hline
Planning Document & 865-line implementation plan with gap analysis, tech stack, task breakdown & Complete \\
Backend Handlers & 4 new Lambda API endpoints for X-Ray and Alarms & Planned \\
Component Library & SRE-specific wrappers on \texttt{@citco/core} & Planned \\
Inventory Dashboard & IISS-509 frontend for resource inventory & In Progress \\
Monitoring Dashboard & IISS-919 config-driven monitoring UI & In Progress \\
\hline
\end{tabularx}
\end{table}

Implementation is deferred to the internship extension period
(Sep--Dec~2026), allowing the Grafana AMG dashboard to stabilize in
...
\rowcolors{2}{tablerowA}{tablerowB}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.32\textwidth}>{\raggedright\arraybackslash}p{0.37\textwidth}}
\hline
...
Taken together, these four decisions shaped an architecture that favors
