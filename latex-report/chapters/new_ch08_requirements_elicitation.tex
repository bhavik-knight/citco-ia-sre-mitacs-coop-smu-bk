...
\hline
\endfoot
\rowcolor{tablerowA} Mr.\ Deotale & Manager, CTM (Industry Supervisor) & Primary sponsor; set engineering priorities; escalated dashboard to Critical priority; final acceptance authority \\
\rowcolor{tablerowB} Mr.\ Leung & IT Senior Developer, RPA Platform Lead & RPA expert; delivered Blue Prism and UiPath KT; defined RPA monitoring scope \\
\rowcolor{tablerowA} Ms.\ Labos & IT Developer, SRE Engineer & Delivered golden signals training; reviewed alarm thresholds and dashboard design \\
\rowcolor{tablerowB} IA IT SRE Team & Internal SRE consumers & Primary users of Grafana dashboard and CloudWatch alarm notifications \\
\rowcolor{tablerowA} Meridian Team & Kafka/ECS architecture owners & Provided MSK consumer group structure, ECS cluster topology, OpenSearch integration context \\
\rowcolor{tablerowB} Mr.\ Vadla \& Ms.\ Venkataraman & Co-interns & Parallel development of monitoring features; design review and test validation \\
\hline
\end{tabularx}
...
knowledge-transfer program covering the full breadth of the IA
...
meaningful SLIs. A key insight from training week: the
...
detection under the reactive model, and explicitly specified the target
for the lag alarm evaluation period (NFR2: $\leq$1~minute).

\paragraph{Golden Signals Session (May 25, 2026).}
Ms.\ Labos' dedicated SRE methodology session translated the theoretical
...
\begin{enumerate}
    \item \textbf{Tag-based resource discovery is non-trivial at Citco
          ...
          \textbf{Shared Cloud Artifacts} budget\_code tag, meaning
          a simple budget\_code lookup conflates resources from
          two operationally distinct environments. This
...
\rowcolor{tablerowA} FR9 & Inventory API supports two-stage tag-based discovery for shared budget\_code platforms & Two-stage AppName filter in \texttt{citcoworks}/\texttt{meridian} discoverers \\
\hline
\end{tabularx}
...
non-functional requirements with measurable targets and values achieved.
...
same budget\_code tag value (Shared Cloud Artifacts)
