...
Seven engineering challenges arose during my internship that were significant
...
\subsection{Challenge 1: CloudWatch \texttt{SEARCH()} Visibility Gap}

The \texttt{SEARCH()} function in CloudWatch only returns metrics for AWS
services with recent activity. Lambda functions not invoked within the
lookback window disappear from dashboard panels entirely --- the same root
cause that made the CAIS-Pricing pipeline invisible for 29 days in
IISS-430. I replaced all \texttt{SEARCH()} expressions with CloudWatch
Metric Insights SQL (\texttt{SELECT ... GROUP BY FunctionName}), which
enumerates all deployed functions in the namespace regardless of recent
activity. This resolved the invisibility problem for idle functions at the
cost of losing true percentile statistics in \texttt{GROUP BY} mode ---
leading directly to Challenge~2 below.

\subsection{Challenge 2: Metric Insights SQL Cannot Compute Percentiles}
...
p50 and p99 percentile statistics are not available in this
mode. This became apparent when I attempted to build a Lambda duration latency
distribution panel that would show P50 and P99 across all Lambda functions in a
...
\subsection{Challenge 3: Grafana 10.4 $\to$ 12.4 Mid-Development Upgrade}
...
\subsection{Challenge 4: Secrets Manager 64\,KB Limit}
...
expanded to cover all seven AWS service categories, the generated YAML grew to
...
\subsection{Challenge 5: Shared BudgetCode Tag Ambiguity}
...
BudgetCode values. A single-stage tag filter could not distinguish
...
was the \textbf{two-stage AppName filter} documented in
Section~\ref{sec:inventory_api}: any discoverer operating in the
\textbf{"Shared Cloud Artifacts"} BudgetCode must apply a secondary
AppName check as a mandatory discriminator. This created a tagging
governance obligation --- new resources added without the correct
AppName tag are invisible to the inventory API --- mitigated by a tag
compliance CloudWatch alarm and a documented platform-wide constraint in the
repository README.

\subsection{Challenge 6: IAM Permission Boundary Blocking Subscription Filters}
...
\subsection{Challenge 7: Module-Import Credential Caching (IISS-706 Hotfix)}

The CAIS Deadline hotfix on Jun~8 exposed a fragility in the credential
