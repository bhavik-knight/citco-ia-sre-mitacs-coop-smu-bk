...
This section describes the observability gaps that existed before my co-op placement and the two critical incidents that directly motivated the monitoring program I was assigned to build.
...
under the \texttt{rpacfs1} platform stopped delivering output for a month without a single alert firing. The root cause
was gradual exhaustion of an external API token, which
caused downstream Lambda invocations to drop to zero while upstream functions kept running, making the pipeline
...
unnoticed. A sub-task was assigned directly to me as immediate
remediation and became the starting point for the full monitoring program.
...
\subsection{From Incident to Program}
...
\begin{tabular}{>{\centering\arraybackslash}p{0.15\textwidth} p{0.70\textwidth}}
\hline
\rowcolor{smubrand}\textcolor{white}{\textbf{Work Stream}} & \textcolor{white}{\textbf{Deliverable Scope}} \\
\hline
\rowcolor{tablerowA} WS1 & CloudWatch Lambda Monitoring Dashboard and inventory \\
\rowcolor{tablerowB} WS2 & Centralized Amazon Managed Grafana Dashboard \\
\rowcolor{tablerowA} WS3 & IA Infrastructure Inventory API \\
\rowcolor{tablerowB} WS4 & RPA Production Support Incidents \\
\rowcolor{tablerowA} WS5 & RPA LLM Agent --- Ask Citco RPA \\
\rowcolor{tablerowB} WS6 & Citco Enterprise Custom UI Dashboards \\
\hline
\end{tabular}
\end{table}
...
program for the \textbf{Ask Citco RPA} chatbot --- a RAG pipeline using
