...
This chapter surveys the tools and platforms that underpinned my internship
...
\textit{budget code} model --- five logical environments (\texttt{rpacfs1},
\texttt{ia-bpo}, \texttt{automationhub}, \texttt{meridian}, \texttt{citcoworks})
...
through Amazon SNS. The IISS-487 delivery included 38 CloudWatch alarms provisioned
...
aggregate across all resources of a given type. For example, a single Metric Insights
query can return the average Lambda duration grouped by function name across all 119
functions in the account --- eliminating the need to define one panel per function.
...
repeatable deployments across all five budget code environments with a single
